...
const reports = [
  { id: "r1", month: 8, version: 1, submitted_at: "2026-09-01T09:00:00Z", snapshot: snapshot(8, [{ date: "2026-08-03", hours: 1, code: null }]) },
  { id: "r2", month: 8, version: 2, submitted_at: "2026-09-02T09:00:00Z", snapshot: snapshot(8, [{ date: "2026-08-03", hours: 1.5, code: null }, { date: "2026-08-05", hours: 0, code: "SA" }]) },
  { id: "r3", month: 7, version: 1, submitted_at: "2026-08-01T09:00:00Z", snapshot: snapshot(7, [{ date: "2026-07-31", hours: 2, code: null }]) },
];
...
describe("assembleYearGrid", () => {
  it("fills cells from submitted snapshots and totals hours with codes as 0", () => {
    ...
    expect(july.cells[30]).toEqual({ hours: 2, code: null });
    expect(july.total).toBe(2);
    expect(august.cells[2]).toEqual({ hours: 1.5, code: null });
    expect(august.cells[4]).toEqual({ hours: 0, code: "SA" });
  ...
  it("uses live sessions in live mode regardless of submissions", () => {
    const grid = assembleYearGrid({
      ...
      submittedByMonth: pickSubmittedMonths(reports),
      liveSessions: [
        { session_date: "2026-09-14", hours: 1.25, code: null },
        { session_date: "2027-06-30", hours: 0, code: "H" },
      ],
    });
    expect(grid.columns[2].cells[13]).toEqual({ hours: 1.25, code: null });
    expect(grid.columns[11].cells[29]).toEqual({ hours: 0, code: "H" });
  ...
  it("exports a CSV with one row per recorded day", () => {
    ...
    expect(lines[0]).toBe("date,student,tutor,hours,code,source");
    expect(lines[1]).toBe('2026-07-31,"Rosa, Jr",Maria,2,,submitted v1');
