...
import type { CustomGoal, GoalAchievement, GoalDefinition, Profile, RecurrenceRule, Student } from "@/lib/supabase/database.types";
...
export type StaffStudentRecord = {
  ...
  liveCustomGoals: Pick<CustomGoal, "id" | "label" | "attained" | "attained_on">[];
...
export async function getStaffStudentRecord(studentId: string, requestedFiscalYear?: number): Promise<StaffStudentRecord | null> {
  ...
  const [tutorRes, reportsRes, allReportsRes, sessionsRes, goalsRes, definitionsRes, earliestRes, rulesRes, customGoalsRes] = await Promise.all([
  ...
  for (const r of [tutorRes, reportsRes, allReportsRes, sessionsRes, goalsRes, definitionsRes, earliestRes, rulesRes, customGoalsRes]) {
    if (r.error) throw new Error(r.error.message);
  }
  ...
  return {
    ...
    liveCustomGoals: customGoalsRes.data ?? [],
